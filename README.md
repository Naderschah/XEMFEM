...
FIXME: IMMEDIATELY (after higher priority work is done)
- I think I am carrying many copies of the mesh during simulation if an AMR mesh is computed so that RAM is used 5 fold
- Why does loading the AMR mesh produce better simulations
...
mesh:
  ...
  AMR:
    ...
    #     L2 norm of the normal flux jump across faces (ie degree of derivative discontinuity)
    ...
    verbose: false
    # AMR mesh export toggles
    # Parallel MFEM pieces (amr_mesh.XXXXXX) are always written
...
# - amr_mesh_serial.mesh                     (single serial MFEM mesh, if save_serial=true)
# - amr_mesh_vtu.pvtu + amr_mesh_vtu_*.vtu   (parallel VTU/PVTU export, if save_vtu_parallel=true)
#
# Note:
# MFEM does not provide a native direct writer for Gmsh 2.2 (.msh) or SALOME (.med).
# The easiest path is converting the VTU/PVTU output with meshio.
...
# Plotting Options
# NOTE: Currently plotting is disabled
plotting:
  show_mesh: false
  mesh_edge_width: 0.1
  img_width: 2160
...
# Just how many divisions to do 
interpolate:
  Nx: 1000
  Ny: 1000
  # Handling of FindPoints code==1 (boundary-projected points):
  # nan|0, accept_value|1, average_elements|2
  code1_mode: average_elements
...
Interpolation uses centered-bin sampling: 
...
So points run from min + dx/2 to max - dx/2 (cell centers), so no point is evaluated exactly on the extent coordinates.

For `FindPointsGSLIB` boundary-projected points (`code == 1`), configure `interpolate.code1_mode`:
- `nan` (`0`): set sampled value to `NaN`.
- `accept_value` (`1`): keep the value returned by the direct interpolation/evaluation call.
- `average_elements` (`2`): evaluate slightly on both sides of the interface and average.
