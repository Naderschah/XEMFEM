...
if __name__ == '__main__':
    ...
    imported_shapes = {}  # logical name -> GEOM shape object

    for name, path in xao_files.items():
        imported, shape, sub, groups, fields = geompy.ImportXAO(path)

        # 1) First try: if there are faces, use them directly
        faces = geompy.ExtractShapes(shape, geompy.ShapeType["FACE"], True)

        if faces:
            # Normal case: XAO already contains faces (e.g. BottomPMTs, FieldCage rings)
            main = geompy.MakeCompound(faces)

        else:
            # No faces in the XAO: try to rebuild faces from wires/edges
            new_faces = []

            # 2) Prefer wires if they are present: each wire can give one face
            wires = geompy.ExtractShapes(shape, geompy.ShapeType["WIRE"], True)
            if wires:
                for i, w in enumerate(wires, start=1):
                    try:
                        f = geompy.MakeFace(w, 1)  # 1 = planar
                        new_faces.append(f)
                    except Exception as e:
                        print(f"Warning: MakeFace failed for wire {i} of {name}: {e}")

            else:
                # 3) No wires, but edges exist: assume a single closed loop
                edges = geompy.ExtractShapes(shape, geompy.ShapeType["EDGE"], True)
                if edges:
                    try:
                        f = geompy.MakeFaceWires(edges, 1)
                        new_faces.append(f)
                    except Exception as e:
                        print(f"Warning: MakeWire/MakeFace failed for edges of {name}: {e}")

            if new_faces:
                # Built one or more faces from the edge geometry
                main = geompy.MakeCompound(new_faces)
            else:
                # Final fallback: keep the original (edge-only) shape
                print(f"Warning: {name} remains edge-only; could not reconstruct faces.")
                main = shape

        imported_shapes[name] = main
        geompy.addToStudy(main, name)
    
    # ----------------------------------------------------------------------
    # Build fragmented LXe / GXe / PTFE regions via CUT (2D faces only)
    #   - Fluids:  (LXeCryostat, GXeCryostat) minus (all PTFE + all electrodes)
    #   - PTFE:    all PTFE primitives, minus electrodes (if overlapping)
    #   - Electrodes: unchanged primitives, used only to extract BC edges
    # ----------------------------------------------------------------------

    # Background cryostat envelopes (fluid domains)
    background_names = ["LXeCryostat", "GXeCryostat"]

    # PTFE primitives (will be combined into one PTFERegion)
    ptfe_names = [
        "GateInsulatingFrame",
        "AnodeInsulatingFrame",
        "CathodeInsulatingFrame",
        "TopScreenInsulatingFrame",
    ...
    # Electrode primitives (types we need separate BCs for)
    electrode_base_names = [
        "Bell",
        "Gate",
        "Anode",
        "Cathode",
    ...
    # Add FieldCage_* and FieldCageGuard_* (per-ring) if present
    fieldcage_like = [
        n for n in imported_shapes.keys()
        if n.startswith("FieldCage_") or n.startswith("FieldCageGuard_")
    ]

    electrode_names = electrode_base_names + fieldcage_like

    # Helper: collect shapes that actually exist
    def shapes_from_names(names):
        return [imported_shapes[n] for n in names if n in imported_shapes]

    # Base shapes from XAO imports (already reconstructed as faces)
    background_shapes = shapes_from_names(background_names)   # [LXeCryostat, GXeCryostat]
    ptfe_shapes       = shapes_from_names(ptfe_names)
    electrode_shapes  = shapes_from_names(electrode_names)

    # Expect exactly two background faces (LXe + GXe)
    LXeCryostat = imported_shapes.get("LXeCryostat", None)
    GXeCryostat = imported_shapes.get("GXeCryostat", None)

    # Combine PTFE and electrodes for cutting
    PTFE_all       = geompy.MakeCompound(ptfe_shapes)       if ptfe_shapes       else None
    Electrodes_all = geompy.MakeCompound(electrode_shapes)  if electrode_shapes  else None

    tools_for_fluids = [s for s in (PTFE_all, Electrodes_all) if s is not None]
    Tools_all        = geompy.MakeCompound(tools_for_fluids) if tools_for_fluids else None

    # ----------------------------------------------------------------------
    # Fluids: LXeRegion, GXeRegion = (LXeCryostat, GXeCryostat) minus (PTFE + electrodes)
    # ----------------------------------------------------------------------
    if LXeCryostat is not None and Tools_all is not None:
        LXeRegion = geompy.MakeCut(LXeCryostat, Tools_all)
    else:
        LXeRegion = LXeCryostat

    if GXeCryostat is not None and Tools_all is not None:
        GXeRegion = geompy.MakeCut(GXeCryostat, Tools_all)
    else:
        GXeRegion = GXeCryostat
    ...
    # ----------------------------------------------------------------------
    # PTFERegion: union of PTFE primitives, minus electrodes (if any)
    # ----------------------------------------------------------------------
    if PTFE_all is not None:
        if Electrodes_all is not None:
            PTFERegion = geompy.MakeCut(PTFE_all, Electrodes_all)
        else:
            PTFERegion = PTFE_all
    ...
    # ----------------------------------------------------------------------
    # Boundary-condition edges: BC_<ElectrodeName>
    #   For each electrode primitive E:
    #     collect edges on the intersection of E with
    #     the *original* LXe/GXe/PTFE primitives (before CUT).
    #   SMESH will still use these edges as geometric supports for groups.
    # ----------------------------------------------------------------------

    bc_edge_shapes = {}  # BC name -> GEOM edge or compound of edges

    LXe_base = imported_shapes.get("LXeCryostat", None)
    GXe_base = imported_shapes.get("GXeCryostat", None)
    PTFE_base = PTFE_all  # compound of all PTFE primitives, may be None

    for name in electrode_names:
        if name not in imported_shapes:
            continue

        E = imported_shapes[name]
        bc_edges = []

        for base_region in (LXe_base, GXe_base, PTFE_base):
            if base_region is None:
                continue

            common = geompy.MakeCommon(base_region, E)
            if common is None:
                continue

            # Edges of the intersection curve between electrode surface and region surface
            edges = geompy.ExtractShapes(common, geompy.ShapeType["EDGE"], True)
            bc_edges.extend(edges)

        if not bc_edges:
            print(f"Warning: no BC edges found for electrode '{name}'")
            continue

        if len(bc_edges) == 1:
            bc_shape = bc_edges[0]
        else:
            bc_shape = geompy.MakeCompound(bc_edges)

        bc_name = f"BC_{name}"
        bc_edge_shapes[bc_name] = bc_shape
        geompy.addToStudy(bc_shape, bc_name)

    # ----------------------------------------------------------------------
    # Parent object for meshing: ONLY fragmented LXe, GXe, PTFE domains
    # ----------------------------------------------------------------------
    ...
    print("GEOM: LXe/GXe/PTFE regions (via CUT) + BC edges constructed.")
    ...
    # If you want to re-enable explicit 1D control later, uncomment:
    # algo1d = Mesh_1.Segment()
    # local_wire_length = min(
    #     p.TopScreenWireDiameter,
    #     p.GateWireDiameter,
    #     p.TopStackWireDiameter,
    #     p.CathodeWireDiameter,
    #     p.BottomScreenWireDiameter,
    # )
    # local_wire_length *= math.pi * 2  # ~12 segments around the smallest wire
    ...
    # (groups are defined on geometry; they’ll be populated at Compute())
    # ---------------------------------------------------------------------
    bc_mesh_groups = {}

    for bc_name, bc_shape in bc_edge_shapes.items():
    ...
    if not isDone:
        raise RuntimeError("Mesh_1.Compute() failed; check geometry/algorithms in SMESH.")
