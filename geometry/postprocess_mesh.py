...
- Reads ./config.yaml and, if mesh.autoappend: true, will inject/replace the autogenerated
  ...
  #TODO Change logic inspect the mesh and check the z order of field shaping 
  autogenerate from there, removing the field cage double validation

  # TODO Add probe pointvoltage validation, ie check boundaries after simulation in the mesh
  And see if they indeed have the correct voltages according to the config
"""
...
if len(list(rem_groups.keys())) > 2: 
    raise Exception("A boundary condition specified in the mesh is not assigned a voltage ", list(rem_groups.keys()))
