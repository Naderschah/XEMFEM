...
int main(int argc, char *argv[]) {
  ...
  // was: double bg_cx = 0.0;
  double bg_cx = 0.5 * bg_W;   // ⇒ bg_cx - 0.5*bg_W == 0.0
  double bg_cy = 0.0;
  ...
  int bottom_capacitor = gmsh::model::occ::addRectangle(
    1.0, y_bot_c - 0.5 * cp_H, 0.0, cp_W, cp_H, -1, roundedRadius
  );

  int top_capacitor = gmsh::model::occ::addRectangle(
    1.0, y_top_c - 0.5 * cp_H, 0.0, cp_W, cp_H, -1, roundedRadius
  );

  int dielectric = gmsh::model::occ::addRectangle(
    1.0, diel_y0, 0.0, cp_W, diel_h
  );
 ...
 // ======================= Materials ==========================
  ...
  int ridx = 0;  // 0 = x, 1 = y
  double tol = 1e-1;             // scale-aware tolerance

  std::vector<std::pair<int,int>> curveDimTags;
  gmsh::model::getEntities(curveDimTags, 1); // all curves

  std::vector<int> axisCurves;
  axisCurves.reserve(curveDimTags.size());

  for(const auto &ct : curveDimTags) {
    int ctag = ct.second;

    double x1,y1,z1, x2,y2,z2;
    gmsh::model::getBoundingBox(1, ctag, x1,y1,z1, x2,y2,z2);

    // Extract radial coordinates
    double r1 = (ridx == 0 ? x1 : y1);
    double r2 = (ridx == 0 ? x2 : y2);

    // Curve is on the axis if BOTH endpoints lie at r ≈ 0
    if(std::abs(r1) <= tol && std::abs(r2) <= tol) {
      axisCurves.push_back(ctag);
    }
  }

  if(axisCurves.empty()) {
    std::cerr << "[Axis] FATAL: No axis boundary found. "
              << "Check cfg->solver.axisymmetric_radial_coord_index.\n";
    std::exit(1);
  }

  int axis_id = cfg->solver.axisymmetric_r0_bd_attribute;
  if(axis_id <= 0) {
    std::cerr << "[Axis] FATAL: axisymmetric_r0_bd_attribute must be > 0.\n";
    std::exit(1);
  }

  int physAxis = gmsh::model::addPhysicalGroup(1, axisCurves, axis_id);
  gmsh::model::setPhysicalName(1, physAxis, "Axis_r0");
  std::cout << "[Axis] Marked " << axisCurves.size()
            << " curve(s) as axis (id=" << axis_id << ")\n";

  // Verify bounds at r = 0
std::vector<std::pair<int,int>> curves;
  gmsh::model::getEntities(curves, 1); // dim = 1 = curves

  std::cout << "\n--- Curve Boundary Inspection ---\n";

  for(const auto &ct : curves) {
    int ctag = ct.second;
    std::cout << "Curve " << ctag << ":\n";

    // Get boundary points of this curve
    std::vector<std::pair<int,int>> pts;
    gmsh::model::getBoundary({{1, ctag}}, pts,
                             /*combined*/ false,
                             /*oriented*/ true,
                             /*recursive*/ false);
    ...
    for(const auto &pt : pts) {
      ...
      gmsh::model::getValue(0, ptag, {}, xyz); // <-- CORRECT signature

      if(xyz.size() >= 3) {
        std::cout << "  Point " << ptag
                  << " → (" << xyz[0] << ", " << xyz[1] << ", " << xyz[2] << ")\n";
      } else {
        std::cout << "  Point " << ptag << " → [ERROR: coordinate retrieval failed]\n";
      }
    }
  }
